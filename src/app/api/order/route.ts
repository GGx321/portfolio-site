...
/** Подписи для полей в сообщении в Telegram (русский). */
...
function buildMessage(body: Body): string {
  ...
  if (projectType) {
    lines.push(`Тип: ${getLabel('project_type', projectType)}`);
  }

  const hasDesign = answersMap.has_design;
  if (hasDesign) {
    lines.push(`Дизайн: ${getLabel('has_design', hasDesign)}`);
  }
  ...
  if (body.contactMethod === 'telegram') {
    ...
    if (body.telegramUsername?.trim()) {
      lines.push(`Юзернейм: @${body.telegramUsername.replace(/^@/, '')}`);
    }
